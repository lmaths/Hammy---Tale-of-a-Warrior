...
func move_platform():
	...
	platform_tween.tween_property(self, "follow", move_direction, durantion).set_delay(WAIT_DURATION)
	platform_tween.tween_property(self, "follow", Vector2.ZERO, durantion).set_delay(durantion + WAIT_DURATION * 2)
